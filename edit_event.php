<?php
session_start();
include 'db_connect.php';
$errors=[];


if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_event'])) {

    $event_id = $_POST['event_id'];
    $title = $_POST['title'];
    $description = $_POST['event_description'];
$date = date("Y-m-d H:i:s", strtotime($_POST['event_date']));
$end_date = date("Y-m-d H:i:s", strtotime($_POST['end_date']));
    $capacity = $_POST['event_capacity'];
    $status = $_POST['event_status'];
    $contact = $_POST['contact_number'];
   $venue_id = !empty($_POST['venue_id']) ? (int)$_POST['venue_id'] : null;
    $club_id = !empty($_POST['club_id']) ? (int)$_POST['club_id'] : null;


  $eventStmt = $conn->prepare("
        SELECT * 
        FROM events 
        WHERE id = ?
    ");
    $eventStmt->bind_param("i", $event_id);
    $eventStmt->execute();
    $currentEvent = $eventStmt->get_result()->fetch_assoc();
    $eventStmt->close();

$isAccepted = ($currentEvent['approval_status'] === 'approved');
$isPending = ($currentEvent['approval_status'] === 'pending');
    


if ($isPending) {

    // Check that end date is after start date
    if ($end_date <= $date) {
        $errors[] = "End date must be after start time.";
    }

    // Check for overlapping events at the same venue
    if (!empty($venue_id)) {
        $overlapSql = $conn->prepare("
            SELECT events.title, events.event_date, events.end_date, venue.venue_name
            FROM events
            LEFT JOIN venue ON events.venue_id = venue.venue_id
            WHERE events.venue_id = ?
              AND events.id != ?
              AND events.approval_status != 'rejected'
              AND (events.event_date < ? AND events.end_date > ?)
        ");
        $overlapSql->bind_param("iiss", $venue_id, $event_id, $end_date, $date);
        $overlapSql->execute();
        $overlapResult = $overlapSql->get_result();

        if ($overlapResult->num_rows > 0) {
            $overlappingEvents = [];
            while ($row = $overlapResult->fetch_assoc()) {
                $overlappingEvents[] = $row;
            }

            $message = "This event overlaps with the following events at the same venue: ";
            foreach ($overlappingEvents as $event) {
                $message .= htmlspecialchars($event['title']) . " | ";
                $message .= date("M d, Y H:i", strtotime($event['event_date'])) . " - ";
                $message .= date("M d, Y H:i", strtotime($event['end_date'])) . " | ";
                $message .= htmlspecialchars($event['venue_name'] ?? 'N/A');
            }

            $errors[] = $message;
        }

        $overlapSql->close();
    }
}

    $event_image = null;
    if (!empty($_FILES['event_image']['name'])) {
        $fileName = $_FILES['event_image']['name'];
        $tempName = $_FILES['event_image']['tmp_name'];
        $ext = strtolower(pathinfo($fileName, PATHINFO_EXTENSION));
        $allowedTypes = ["jpg", "jpeg", "png", "gif"];
        if (!in_array($ext, $allowedTypes)) {
            $errors[] = "Invalid image type. Allowed: JPG, JPEG, PNG, GIF.";
        } else {
            $uniqueName = time() . "_" . bin2hex(random_bytes(4)) . "." . $ext;
            $targetPath = "uploads/" . $uniqueName;
            if (!move_uploaded_file($tempName, $targetPath)) {
                $errors[] = "Failed to upload event image.";
            } else{
                $event_image = $uniqueName;
            }
            }
        }


    if (!empty($errors)) {
        $_SESSION['form_errors'] = $errors;
        header("Location: organizer_event.php");
        exit;
    }

    
    if ($isAccepted) {
        
        if ($event_image !== null) {
            $stmt = $conn->prepare("UPDATE events SET event_image=?, contact_number=?, venue_id=? WHERE id=?");
            $stmt->bind_param("ssii", $event_image, $contact, $venue_id, $event_id);
        } else {
            $stmt = $conn->prepare("UPDATE events SET contact_number=?, venue_id=? WHERE id=?");
            $stmt->bind_param("sii", $contact, $venue_id, $event_id);
        }
    } else {

    if ($event_image !==null) {
        $stmt = $conn->prepare("UPDATE events SET title=?, event_description=?, event_date=?, end_date=?, event_capacity=?, event_image=?, event_status=?, contact_number=?, venue_id=?,club_id=? WHERE id=?");
        $stmt->bind_param("ssssisisiii", $title, $description, $date, $end_date, $capacity, $event_image, $status, $contact, $venue_id, $club_id, $event_id);
    } else {
        $stmt = $conn->prepare("UPDATE events SET title=?, event_description=?, event_date=?, end_date=?, event_capacity=?, event_status=?, contact_number=?, venue_id=?,club_id=? WHERE id=?");
        $stmt->bind_param("ssssissiii", $title, $description, $date, $end_date, $capacity, $status, $contact, $venue_id,$club_id, $event_id);
    }
}

    if ($stmt->execute()) {
        $stmt->close();
        header("Location: organizer_event.php?message=updated");
        exit;
    } else {
        $_SESSION['form_errors'] = ["Failed to update event."];
        header("Location: organizer_event.php");
        exit;
    }
}
?>